...
try:
 ...
 for out_path in evidence.get('evidence_outputs',[]):
  # Validation reports and the current release package checksum are generated by this validation run.
  generated_outputs={'validation/validation-report.json',f'packages/gaam-v{RELEASE_VERSION}/checksums.json'}
  if out_path not in generated_outputs and not (ROOT/out_path).exists(): status_errors.append(f'missing declared evidence output {out_path}')
